...
export class BookService extends BaseHttpClient {
  async getBooks() {
    return this.get<ApiResponse<BookResponse>>(BOOK_LIST_PATH).then((res) => res);
  }

  async getSharedBooks() {
    return this.get<ApiResponse<BookResponse>>(BOOK_SHARED_LIST_PATH).then((res) => res);
  }
  ...
  async createBook(payload: CreateBookPayload) {
    return this.post<ApiResponse<BookResponse>>(BOOK_CREATE_PATH, payload).then((res) => res);
  }

  async deleteBook(bookId: string) {
    return this.delete<ApiResponse<null>>(replaceString(BOOK_DELETE_PATH, { ':bookId': bookId })).then((res) => res);
  }
}
